...
obj1 %>% select(c('KOH.cat','age','Sex.long','avg.bmi','koh.counts','HTN','PreDM','T2DM','both','ClinicLocation','No.column.name','IncomeLevel','BLACERISK')) %>%
   gtsummary::tbl_summary(
     by = KOH.cat,
     include = c(age, Sex.long, avg.bmi, koh.counts, HTN, PreDM, T2DM, both, ClinicLocation, No.column.name, IncomeLevel, BLACERISK),
     label = list(
       age ~ 'Age',
       Sex.long ~ 'Sex',
       avg.bmi ~ 'BMI',
...
```
```{r}
high.bmi <- bmi.nona.18 %>% filter(UniqueIdentifier == '335835' | UniqueIdentifier == '463060' | UniqueIdentifier == '316975' | UniqueIdentifier == '49203')
...
```{r echo=FALSE, warning=FALSE}
table1 %>% select(c('Group','age','Sex.long','avg.bmi','HTN','PreDM','T2DM','both','ClinicLocation','No.column.name','IncomeLevel','BLACERISK')) %>%
   gtsummary::tbl_summary(
     by = Group,
