...
table 1 for KOH only
```{r echo=FALSE, warning=FALSE}
koh.table1 %>% select(c('age','Sex.long','koh.counts','HTN','PreDM','T2DM','both','ClinicLocation','No.column.name','IncomeLevel','BLACERISK')) %>%
   gtsummary::tbl_summary(
     by = NULL,
     include = c(age, Sex.long, koh.counts, HTN, PreDM, T2DM, both, ClinicLocation, No.column.name, IncomeLevel, BLACERISK),
     label = list(
       age ~ 'Age',
       Sex.long ~ 'Sex',
...
```
## Primary Objective - pre and post analysis datasets ##
...
write.csv(obj1.bp.pre.post, 'Obj1BPPrePost.csv')
```
...
obj1.pre.post <- bind_rows(koh.a1c.pre.post_long, marsh.non.a1c.pre.post_long)
obj1.pre.post$KOH[is.na(obj1.pre.post$KOH)] <- 0

#write.csv(obj1.pre.post, 'Obj1A1cPrePost.csv')
```
*** next on Gabby's TO DO list is constructing the analysis data set for the longitudinal KOH analysis ***

table 1 for all Marshallese and non-Marshallese
```{r echo=FALSE, warning=FALSE}
table1 %>% select(c('Group','age','Sex.long','HTN','PreDM','T2DM','both','ClinicLocation','No.column.name','IncomeLevel','BLACERISK')) %>%
   gtsummary::tbl_summary(
     by = Group,
     include=c(age, Sex.long, HTN, PreDM, T2DM, both, ClinicLocation, No.column.name, IncomeLevel, BLACERISK),
     list(
       age ~ 'Age',
       Sex.long ~ 'Sex',
       HTN ~ 'Hypertension',
...
enc.race$year <- as.factor(as.numeric(format(enc.race$Date, "%Y")))
enc.race$Group <- factor(enc.race$Group, levels = c("Marshallese","Non-Marshallese"))
...
enc.race$ServiceLine <- factor(enc.race$ServiceLine,
                               levels = c("Primary Care", "Emergency", "Urgent Care", "Behavioral Health",
                                          "Pharmacy", "Health Equity", "Dental", "Multispecialty", "Other"))# calculate number of type of visit per year for each patient
...
enc.counts.wide %>%
  select(year, Group, PrimaryCare, Emergency, UrgentCare, Behavioral, Pharmacy, Dental, Multispecialty) %>%
  filter(year != "2016" & year != "2017" & year != "2018" & year != "2019" & year != "2025") %>%
  mutate(across(c(PrimaryCare, Emergency, UrgentCare, Behavioral, Pharmacy, 
