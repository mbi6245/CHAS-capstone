...
obj1_res = data.frame(
  rbind(
    cbind(a1c.ci1, a1c.p1),
    cbind(bp.ci1, bp.p1)
  )
)

# obj1_res <- obj1_res %>% mutate(ci = cat("(",lower, ", ", upper, ")", sep=""))
# obj1_res = data.frame(
...
colnames(obj1_res) = c("estimate ", "  95% CI", "p-value")

library(kableExtra)

# Display the table
kable(obj1_res, align = c("c", "c", "c", "c")) %>%
  pack_rows("A1c Longitudinal", 1, 5) %>%
  pack_rows("SBP Longitudinal", 6, 10) %>%
